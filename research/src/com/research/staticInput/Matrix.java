...
//	}
	...
	public Matrix matrixM(Matrix A, Matrix identity, Matrix negA, int total) {
		Matrix M = new Matrix(total, total);
		int fix = identity.column;
		int sum = fix + A.row;
		...
		for (int i = 0; i < total; i++) {
			for (int j = 0; j < total; j++) {
				if (j >= fix) {
					...
					if (i < fix && j < fix) {
						M.element[i][j] = identity.element[i][j];
					} else if (i >= fix && i < (A.row + identity.row)) {
						M.element[i][j] = A.element[i - fix][j];
					} else if (i < total && i >= (A.row + identity.row)) {
...
//	}
	...
	// convert 2d matrix to 1 d array
	...
	//convert 1d array to 2 d array
		public double[][] oneDto2dArray(double[] any, int row, int column) {
			
			double[][] arrayArray = new double[row][column];
			for (int i = 0; i < row; i++) {
				for (int j = 0; j < column; j++) {
					arrayArray[i][j] = any[j];
				}
			}
			return arrayArray;
	...
	//convert 2d array to matrix
		public Matrix arrayToMatrix(double[][] anyArray) {
			Matrix arrayToMatrix= new Matrix(anyArray);
